...
get_download_metrics_cumulative <- function(download_data_path, cadence, start, end) {
  ...
  if (cadence == "weekly") {
    ...
    # calculate cumulative metrics for one week
    get_one_week <- function(week_start) {
      
      week_end <- week_start + lubridate::days(7)
      
      # cumulative data:
      # starts at the overall start date and runs through this week's end
      downloads <- downloads_all %>%
        filter(
          created >= start_week,
          created < week_end
    ...
    # calculate cumulative metrics for every week
    cumulative_metrics <- bind_rows(
      lapply(week_starts, get_one_week)
    )
...
download_metrics_cumulative <- get_download_metrics_cumulative(
  download_data_path = "~/Desktop/download_events_0822.csv",
  cadence = "weekly",
  start = "2026-08-09",
  end = "2026-08-23"
