...
def get_limbo_registrations():
    ...
    COL_HEADERS = ['reg_guid', 'is_public', 'is_deleted', 'spam_status', 'date_created', 'date_registered', 
                   'moderation_state', 'registration_approval_state', 'embargo_state', 'embargo_end_date', 'retraction_state']
    output = io.StringIO()
    writer = csv.DictWriter(output, COL_HEADERS)
    writer.writeheader()

    regs = Registration.objects.select_related('embargo', 'retraction', 'registration_approval').all()
    ...
    for reg in regs.iterator(chunk_size=1000):
        writer.writerow({
            ...
            'registration_approval_state': reg.registration_approval.state if reg.registration_approval else None,
