...
all_reg <- read_csv("~/Desktop/all_registrations_for_los_final_2026-03-04.csv")
...
updated_master_without_funders <- updated_master_wtotals %>%
  filter(dimension != "funder" & dimension != "funded")

write_sheet(updated_master_without_funders, los_sheet_url, sheet = "Master")

# no longer need to add funder_type to Funder tab
