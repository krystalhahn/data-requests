...
@timeit
def get_quarterly_inst_metrics_optimized(year, quarter):
    """
    Generate a report of institutional metrics for a given quarter.

    This optimized version reduces the number of database queries by bulk fetching and precomputing user-related metrics.
    ...
    Example usage:
    --------------
    get_quarterly_inst_metrics_optimized(2023, 2)
    ...
    from osf.models import Institution, OSFUser, Preprint, NodeLog, PreprintLog

    filename = "/tmp/institutional_metrics_optimized.csv"
    COL_HEADERS = [
        "institution.name",
        "total_users",
        "active_users",
    ...
    start_dt = timezone.datetime(year, start_month, 1, tzinfo=pytz.utc)
    end_dt = (
        timezone.datetime(year + 1, 1, 1, tzinfo=pytz.utc)
        if end_month == 12
        else timezone.datetime(year, end_month + 1, 1, tzinfo=pytz.utc)
    )
    ...
    users = (
        OSFUser.objects.filter(
            institutionaffiliation__institution__id__in=inst_ids, is_active=True
        )
        .exclude(spam_status=2)
        .distinct()
        .prefetch_related("logs", "preprint_logs")
    )
    ...
    orcid_verified_map = {
        u.id: ("VERIFIED" in u.external_identity.get("ORCID", {}).values())
        for u in users
    }
    ...
    quarterly_login_map = {
        u.id: u.date_last_login is not None and start_dt <= u.date_last_login < end_dt
        for u in users
    }
    ...
    preprint_actions = Counter(
        PreprintLog.objects.filter(
            user__in=users, created__gte=start_dt, created__lt=end_dt
        ).values_list("user_id", flat=True)
    )
    user_actions_map = {
        u.id: node_actions.get(u.id, 0) + preprint_actions.get(u.id, 0) for u in users
    }
    ...
    for inst in insts:
        ...
        domain_metrics = {
            ...
            "total_preprints": Preprint.objects.filter(
                _contributors__in=inst_users, is_public=True, is_published=True
            )
            .exclude(spam_status=2)
            .distinct()
            .count(),
            "public_top_projects": inst.nodes.filter(
                type="osf.node", is_public=True, deleted__isnull=True
            )
            .exclude(spam_status=2)
            .get_roots()
            .count(),
            "private_top_projects": inst.nodes.filter(
                type="osf.node", is_public=False, deleted__isnull=True
            )
            .exclude(spam_status=2)
            .get_roots()
            .count(),
            "public_projects": inst.nodes.filter(
                type="osf.node", is_public=True, deleted__isnull=True
            )
            .exclude(spam_status=2)
            .count(),
            "private_projects": inst.nodes.filter(
                type="osf.node", is_public=False, deleted__isnull=True
            )
            .exclude(spam_status=2)
            .count(),
            "public_registrations": inst.nodes.filter(
                type="osf.registration", is_public=True, deleted__isnull=True
            )
            .exclude(spam_status=2)
            .count(),
            "private_registrations": inst.nodes.filter(
                type="osf.registration", is_public=False, deleted__isnull=True
            )
            .exclude(spam_status=2)
            .count(),
            "embargoed_registrations": inst.nodes.filter(
                type="osf.registration",
                is_public=False,
                deleted__isnull=True,
                embargo__state="approved",
